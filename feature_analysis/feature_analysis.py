...
for r, csvRecord in enumerate(csvLines):
	...
	for u, user in enumerate(user_email_map.user_array):
...
statsSheet.write_row("A2", ["Min Sent", "=MIN(SentEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1) + ")"]);
statsSheet.write_row("A3", ["Average Sent", "=AVERAGE(SentEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1) + ")"]);
statsSheet.write_row("A5", ["Max Received", "=MAX(ReceivedEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1) + ")"]);
statsSheet.write_row("A6", ["Min Received", "=MIN(ReceivedEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1) + ")"]);
statsSheet.write_row("A7", ["Average Received", "=AVERAGE(SentEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1) + ")"]);

#Create sent histogram
chart = excelFile.add_chart({"type": "column"});
chart.add_series({
    "name":       "=SentEmailCounts!$B$1",
    "categories": "=SentEmailCounts!$A$2:$A$" + str(len(user_email_map.user_array) + 1),
    "values":     "=SentEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1),
});
...
chart.add_series({
    ...
    "categories": "=ReceivedEmailCounts!$A$2:$A$" + str(len(user_email_map.user_array) + 1),
    "values":     "=ReceivedEmailCounts!$B$2:$B$" + str(len(user_email_map.user_array) + 1),
});
