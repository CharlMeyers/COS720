...
excelFile = xlsxwriter.Workbook("./output/Received_emails.xlsx");
...
emailCountsSheet.write_row("A1", ["User", "Number of Received emails"]);
receivedTimeSheet = excelFile.add_worksheet("SentTime");
receivedTimeSheet.write_row("A1", ["Hour", "Count"]);
...
for i, user in enumerate(os.listdir("../maildir")):
	...
	userReceivedPath = userPath + "/inbox/";

	try:
		receivedFileNames = os.listdir(userReceivedPath);
		print(user + " received " + str(len(receivedFileNames)) + " emails.");
		emailCountsSheet.write_row("A" + str(i + 2), [user, len(receivedFileNames)]);

		for j, emailName in enumerate(receivedFileNames):
			emailPath = userReceivedPath + emailName;
...
for t, timeTuple in enumerate(timeArray):
	receivedTimeSheet.write_row("A" + str(t + 2), [timeTuple[0], timeTuple[1]]);
